...
FixRigidNC::FixRigidNC(LAMMPS *lmp, int narg, char **arg) :
...
void FixRigidNC::setup_pre_neighbor()
{
  pre_neighbor();
}
...
void FixRigidNC::setup(int vflag)
{
  int i,n,ibody;
...
void FixRigidNC::initial_integrate(int vflag)
{
  double dtfm;
...
void FixRigidNC::apply_langevin_thermostat()
{
  if (me == 0) {
...
void FixRigidNC::compute_forces_and_torques()
{
  int i,ibody;
...
void FixRigidNC::post_force(int /*vflag*/)
{
...
void FixRigidNC::final_integrate()
{
  int ibody;
  double dtfm;
...
void FixRigidNC::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixRigidNC::final_integrate_respa(int ilevel, int /*iloop*/)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
void FixRigidNC::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nbody; ibody++)
...
void FixRigidNC::setup_bodies_static()
{
  int i,ibody;
...
void FixRigidNC::setup_bodies_dynamic()
{
  int i,ibody;
...
void FixRigidNC::readfile(int which, double *vec,
...
void FixRigidNC::write_restart_file(char *file)
{
  if (me) return;
...
double FixRigidNC::memory_usage()
{
  int nmax = atom->nmax;
...
void FixRigidNC::grow_arrays(int nmax)
{
...
void FixRigidNC::copy_arrays(int i, int j, int /*delflag*/)
{
  body[j] = body[i];
...
void FixRigidNC::set_arrays(int i)
{
  body[i] = -1;
  xcmimage[i] = 0;
  displace[i][0] = 0.0;
...
int FixRigidNC::pack_exchange(int i, double *buf)
{
  buf[0] = ubuf(body[i]).d;
...
int FixRigidNC::unpack_exchange(int nlocal, double *buf)
{
...
int FixRigidNC::modify_param(int narg, char **arg)
{
...
double FixRigidNC::compute_scalar()
{
...
void *FixRigidNC::extract(const char *str, int &dim)
{
...
double FixRigidNC::extract_erotational()
{
  double wbody[3],rot[3][3];
...
double FixRigidNC::compute_array(int i, int j)
{
