...
FixStyle(rigid/big,FixRigidBig)

#else

#ifndef LMP_FIX_RIGID_BIG_H
#define LMP_FIX_RIGID_BIG_H

#include "fix.h"

namespace LAMMPS_NS {

class FixRigidBig : public Fix {
  friend class ComputeRigidLocal;

 public:
  FixRigidBig(class LAMMPS *, int, char **);
  virtual ~FixRigidBig();
  virtual int setmask();
  virtual void init();
...
E: Fix rigid/big requires atom attribute molecule

Self-explanatory.

E: Fix rigid/big custom requires previously defined property/atom

UNDOCUMENTED

E: Fix rigid/big custom requires integer-valued property/atom

UNDOCUMENTED

E: Variable name for fix rigid/big custom does not exist

UNDOCUMENTED

E: Fix rigid/big custom variable is no atom-style variable
...
E: Fix rigid/big requires an atom map, see atom_modify

Self-explanatory.

E: Fix rigid/big langevin period must be > 0.0

Self-explanatory.

E: Molecule template ID for fix rigid/big does not exist

Self-explanatory.

E: Fix rigid/big nvt/npt/nph dilate group ID does not exist

Self-explanatory.

E: Fix rigid/big molecule must have coordinates

The defined molecule does not specify coordinates.

E: Fix rigid/big molecule must have atom types
...
E: Fix rigid/big atom has non-zero image flag in a non-periodic dimension
...
E: Cannot open fix rigid/big infile %s

The specified file cannot be opened.  Check that the path and name are
correct.

E: Unexpected end of fix rigid/big file

A read operation from the file failed.

E: Incorrect rigid body format in fix rigid/big file

The number of fields per line is not what expected.

E: Invalid rigid body ID in fix rigid/big file

The ID does not match the number of an existing ID of rigid bodies
that are defined by the fix rigid/big command.
...
to encompass the max distance printed when the fix rigid/big command
was invoked.
