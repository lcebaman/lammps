...
FixRigidBig::FixRigidBig(LAMMPS *lmp, int narg, char **arg) :
  ...
  id_dilate(NULL), onemols(NULL)
{
  ...
  if (narg < 4) error->all(FLERR,"Illegal fix rigid/big command");
  if (strcmp(arg[3],"molecule") == 0) {
    if (atom->molecule_flag == 0)
      error->all(FLERR,"Fix rigid/big requires atom attribute molecule");
    bodyID = atom->molecule;

  } else if (strcmp(arg[3],"custom") == 0) {
    if (narg < 5) error->all(FLERR,"Illegal fix rigid/big command");
      ...
      if (strstr(arg[4],"i_") == arg[4]) {
        ...
        if (custom_index == -1)
          error->all(FLERR,"Fix rigid/big custom requires "
        ...
        if (ivariable < 0)
          error->all(FLERR,"Variable name for fix rigid/big custom "
                     "does not exist");
        if (input->variable->atomstyle(ivariable) == 0)
          error->all(FLERR,"Fix rigid/big custom variable is not "
                     "atom-style variable");
  ...
  } else error->all(FLERR,"Illegal fix rigid/big command");

  if (atom->map_style == 0)
    error->all(FLERR,"Fix rigid/big requires an atom map, see atom_modify");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"langevin") == 0) {
      if (iarg+5 > narg) error->all(FLERR,"Illegal fix rigid/big command");
      if ((strcmp(style,"rigid/big") != 0) &&
          (strcmp(style,"rigid/nve/big") != 0) &&
          (strcmp(style,"rigid/nph/big") != 0))
        error->all(FLERR,"Illegal fix rigid/big command");
      langflag = 1;
      ...
      if (t_period <= 0.0)
        error->all(FLERR,"Fix rigid/big langevin period must be > 0.0");
      if (seed <= 0) error->all(FLERR,"Illegal fix rigid/big command");
      iarg += 5;

    } else if (strcmp(arg[iarg],"infile") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rigid/big command");
      delete [] infile;
      ...
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix rigid/big command");
      if (strcmp(style,"rigid/nvt/big") != 0 &&
          strcmp(style,"rigid/npt/big") != 0)
      ...
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix rigid/big command");
      if (strcmp(style,"rigid/npt/big") != 0 &&
          strcmp(style,"rigid/nph/big") != 0)
              error->all(FLERR,"Illegal fix rigid/big command");
      pcouple = XYZ;
      ...
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal fix rigid/big nvt/npt/nph command");
      if (strcmp(arg[iarg+1],"all") == 0) allremap = 1;
        ...
        if (idilate == -1)
          error->all(FLERR,"Fix rigid/big nvt/npt/nph dilate group ID "
                     "does not exist");
      }
      iarg += 2;
  ...
  if (onemols) {
    for (int i = 0; i < nmol; i++) {
      if (onemols[i]->xflag == 0)
        error->all(FLERR,"Fix rigid/big molecule must have coordinates");
      if (onemols[i]->typeflag == 0)
        error->all(FLERR,"Fix rigid/big molecule must have atom types");

      // fix rigid/big uses center, masstotal, COM, inertia of molecule
...
void FixRigidBig::setup_pre_neighbor()
{
...
void FixRigidBig::setup(int vflag)
{
  int i,n,ibody;
...
void FixRigidBig::initial_integrate(int vflag)
{
  double dtfm;
...
void FixRigidBig::apply_langevin_thermostat()
{
  ...
  if (nlocal_body > maxlang) {
    ...
    memory->create(langextra,maxlang,6,"rigid/big:langextra");
  }
...
void FixRigidBig::post_force(int /*vflag*/)
{
...
void FixRigidBig::compute_forces_and_torques()
{
  int i,ibody;
...
void FixRigidBig::final_integrate()
{
  double dtfm;
...
void FixRigidBig::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixRigidBig::final_integrate_respa(int ilevel, int /*iloop*/)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
int FixRigidBig::dof(int tgroup)
{
  int i,j;
  ...
  memory->create(counts,nlocal_body+nghost_body,3,"rigid/big:counts");
...
void FixRigidBig::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nlocal_body; ibody++)
...
void FixRigidBig::create_bodies(tagint *bodyID)
{
  int i,m;
  ...
  int *proclist;
  memory->create(proclist,ncount,"rigid/big:proclist");
  InRvous *inbuf = (InRvous *)
    memory->smalloc(ncount*sizeof(InRvous),"rigid/big:inbuf");
...
int FixRigidBig::rendezvous_body(int n, char *inbuf,
                                   ...
                                   void *ptr)
{
  ...
  FixRigidBig *frsptr = (FixRigidBig *) ptr;
  ...
  memory->create(bbox,ncount,6,"rigid/big:bbox");
  ...
  memory->create(ctr,ncount,3,"rigid/big:bbox");
  ...
  memory->create(idclose,ncount,"rigid/big:idclose");
  memory->create(iclose,ncount,"rigid/big:iclose");
  memory->create(rsqclose,ncount,"rigid/big:rsqclose");
  ...
  int nout = n;
  memory->create(proclist,nout,"rigid/big:proclist");
  OutRvous *out = (OutRvous *)
    memory->smalloc(nout*sizeof(OutRvous),"rigid/big:out");
...
void FixRigidBig::setup_bodies_static()
{
  int i,ibody;
  ...
  int *inbody;
  if (infile) {
    memory->create(inbody,nlocal_body,"rigid/big:inbody");
  ...
  memory->create(itensor,nlocal_body+nghost_body,6,"rigid/big:itensor");
  for (ibody = 0; ibody < nlocal_body+nghost_body; ibody++)
...
void FixRigidBig::setup_bodies_dynamic()
{
  int i,ibody;
...
/* ----------------------------------------------------------------------
   ...
   where rigid-ID = mol-ID for fix rigid/big
------------------------------------------------------------------------- */

void FixRigidBig::readfile(int which, double **array, int *inbody)
{
  ...
  if (me == 0) {
    ...
    if (fp == NULL) {
      char str[128];
      snprintf(str,128,"Cannot open fix rigid/big infile %s",infile);
      error->one(FLERR,str);
    }
    ...
    while (1) {
      ...
      if (eof == NULL)
        error->one(FLERR,"Unexpected end of fix rigid/big file");
  ...
  while (nread < nlines) {
    ...
    if (eofflag) error->all(FLERR,"Unexpected end of fix rigid/big file");
    ...
    *next = '\n';
    ...
    if (nwords != ATTRIBUTE_PERBODY)
      error->all(FLERR,"Incorrect rigid body format in fix rigid/big file");
    ...
    for (int i = 0; i < nchunk; i++) {
      ...
      if (id <= 0 || id > maxmol)
        error->all(FLERR,"Invalid rigid body ID in fix rigid/big file");
...
void FixRigidBig::write_restart_file(char *file)
{
  FILE *fp;
  ...
  double **buf;
  if (me == 0) memory->create(buf,MAX(1,maxrow),ncol,"rigid/big:buf");
  else memory->create(buf,MAX(1,sendrow),ncol,"rigid/big:buf");
...
void FixRigidBig::grow_arrays(int nmax)
{
  memory->grow(bodyown,nmax,"rigid/big:bodyown");
  memory->grow(bodytag,nmax,"rigid/big:bodytag");
  memory->grow(atom2body,nmax,"rigid/big:atom2body");
  memory->grow(xcmimage,nmax,"rigid/big:xcmimage");
  memory->grow(displace,nmax,3,"rigid/big:displace");
  if (extended) {
    memory->grow(eflags,nmax,"rigid/big:eflags");
    if (orientflag) memory->grow(orient,nmax,orientflag,"rigid/big:orient");
    if (dorientflag) memory->grow(dorient,nmax,3,"rigid/big:dorient");
  }
...
void FixRigidBig::copy_arrays(int i, int j, int delflag)
{
  bodytag[j] = bodytag[i];
...
void FixRigidBig::set_arrays(int i)
{
  bodyown[i] = -1;
  bodytag[i] = 0;
  atom2body[i] = -1;
...
void FixRigidBig::set_molecule(int nlocalprev, tagint tagprev, int imol,
...
int FixRigidBig::pack_exchange(int i, double *buf)
{
...
int FixRigidBig::unpack_exchange(int nlocal, double *buf)
{
...
int FixRigidBig::pack_forward_comm(int n, int *list, double *buf,
...
void FixRigidBig::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,last;
  double *xcm,*vcm,*quat,*omega,*ex_space,*ey_space,*ez_space,*conjqm;
...
int FixRigidBig::pack_reverse_comm(int n, int first, double *buf)
{
  int i,j,m,last;
...
void FixRigidBig::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,k;
  double *fcm,*torque,*vcm,*angmom,*xcm;
...
void FixRigidBig::reset_atom2body()
{
  int iowner;
...
void FixRigidBig::zero_momentum()
{
  double *vcm;
...
void FixRigidBig::zero_rotation()
{
  double *angmom,*omega;
  for (int ibody = 0; ibody < nlocal_body+nghost_body; ibody++) {
    angmom = body[ibody].angmom;
...
int FixRigidBig::modify_param(int narg, char **arg)
{
...
void *FixRigidBig::extract(const char *str, int &dim)
{
...
double FixRigidBig::extract_erotational()
{
...
double FixRigidBig::compute_scalar()
{
  double wbody[3],rot[3][3];
...
double FixRigidBig::memory_usage()
{
  int nmax = atom->nmax;
...
/*
void FixRigidBig::check(int flag)
{
